...
function countGeneralUnread(userId) {
  const readAt = generalReadAt.get(userId);
  if (!readAt) return generalMessages.length;
  return generalMessages.filter((m) => m.timestamp > readAt).length;
}
...
function buildSyncState(userId) {
  ...
  return {
    generalUnread: countGeneralUnread(userId),
    dmUnreadByPeer,
...
function registerSocketHandlers(io) {
  io.on("connection", (socket) => {
    ...
    socket.emit("general:history", generalMessages.slice(-100));
    ...
    socket.on("general:mark_read", () => {
      const lastTs =
        generalMessages.length > 0
          ? generalMessages[generalMessages.length - 1].timestamp
          : new Date().toISOString();
      generalReadAt.set(myId, lastTs);
      emitToUser(io, myId, "general:unread", { count: 0 });
    });

    socket.on("general:fetch", ({ before, limit = 50 } = {}) => {
      if (typeof before !== "string") {
        return socket.emit("general:page", { messages: [], hasMore: false });
      }
      const pool = generalMessages.filter((m) => m.timestamp < before);
      const slice = pool.slice(-limit);
      socket.emit("general:page", {
        messages: slice,
    ...
    socket.on("message:send", (data) => {
      if (!systemConfig.featureFlags.general) {
        return socket.emit("chat:error", { message: "General chat is disabled." });
      }
      if (systemConfig.chatFrozen && me.username !== "admin") {
        return socket.emit("chat:error", { message: "Chat is frozen by moderators." });
      }
      if (!data?.text || typeof data.text !== "string") return;
      let trimmed = data.text.trim();
      if (!trimmed) return;
      const maxLen = systemConfig.maxMessageLength || 2000;
      if (trimmed.length > maxLen) {
        return socket.emit("chat:error", { message: `Message too long (max ${maxLen}).` });
      }
      const nowMs = Date.now();
      const lastRl = rateLimitGeneral.get(myId) || 0;
      if (nowMs - lastRl < (systemConfig.rateLimitGlobalMs || 300)) {
        return socket.emit("chat:error", { message: "You are sending messages too fast." });
      }
      const slowSec = systemConfig.slowModeSeconds || 0;
      if (slowSec > 0 && me.username !== "admin") {
        const lastPost = slowModeLastPost.get(myId) || 0;
        if (nowMs - lastPost < slowSec * 1000) {
          return socket.emit("chat:error", { message: `Slow mode: wait ${slowSec}s between messages.` });
        }
        slowModeLastPost.set(myId, nowMs);
      }
      rateLimitGeneral.set(myId, nowMs);
      for (const w of profanityWords) {
        const word = String(w || "").trim();
        if (word && trimmed.toLowerCase().includes(word.toLowerCase())) {
          return socket.emit("chat:error", { message: "Message blocked by profanity filter." });
        }
      }
      const msg = {
      ...
      if (generalMessages.length > MAX_GENERAL) generalMessages.splice(0, generalMessages.length - MAX_GENERAL);
      io.emit("message:new", msg);
      notifyAdminRoom(io, { type: "general_message", msgId: msg.id });
      for (const [uid] of presence) {
        if (uid === myId) continue;
        const c = countGeneralUnread(uid);
        emitToUser(io, uid, "general:unread", { count: c });
      }
    });

    socket.on("message:react", ({ msgId, emoji } = {}) => {
      if (typeof msgId !== "string" || typeof emoji !== "string" || !emoji.trim()) return;
      const m = generalMessages.find((x) => x.id === msgId);
      if (!m) return;
      ...
      io.emit("message:reaction:update", { msgId, reactions: m.reactions });
    });

    socket.on("message:edit", ({ msgId, text } = {}) => {
      if (typeof msgId !== "string" || typeof text !== "string") return;
      const trimmed = text.trim();
      if (!trimmed) return;
      const m = generalMessages.find((x) => x.id === msgId);
      if (!m || m.userId !== myId) return;
      m.text = trimmed;
      m.edited = true;
    ...
    socket.on("message:delete", ({ msgId } = {}) => {
      ...
      const idx = generalMessages.findIndex((x) => x.id === msgId);
      if (idx < 0) return;
      const m = generalMessages[idx];
      if (m.userId !== myId) return;
      generalMessages.splice(idx, 1);
      io.emit("message:deleted", { msgId });
    });

    socket.on("typing:start", (payload = {}) => {
      const { context = "general", toUserId } = payload;
      if (context === "general") {
