...
export default function ChatLayout({
  ...
  // Lists
  const dmList = useMemo(() => friends.map((f) => { const messages = dmByUserId[f.id] || []; const last = messages[messages.length - 1]; return { friend: f, unread: dmUnread[f.id] || 0, preview: last?.text || "", status: f.status }; }).sort((a, b) => (b.unread > 0 ? 1 : -1)), [friends, dmByUserId, dmUnread]);
  const filteredFriends = useMemo(() => { let list = friends; if (friendFilter === "online") list = list.filter(f => f.status === "online"); if (searchQuery) list = list.filter(f => f.username.toLowerCase().includes(searchQuery.toLowerCase())); return list; }, [friends, friendFilter, searchQuery]);
          ...
          <AnimatePresence mode="wait">
            {sidebarView === "dms" ? (
              ...
              <motion.div key="friends" variants={fadeIn}>
                ...
                {/* Pending Requests */}
                {friendFilter === "pending" && friendRequests.length > 0 && (
                  <>
                    <div className="category-header" style={{ marginTop: 16 }}>INCOMING REQUESTS — {friendRequests.length}</div>
