...
function registerSocketHandlers(io) {
  io.on("connection", (socket) => {
    ...
    // Message Reply - DM
    socket.on("dm:message:reply", async ({ parentMessageId, text, toUserId } = {}) => {
      if (!parentMessageId || !text || !toUserId) return;
      ...
      try {
        ...
        if (error) {
          console.error("[dm:message:reply] Error saving reply:", error);
          return;
        }
        ...
        // Broadcast to other user
        emitToUser(io, toUserId, "dm:message:replied", replyData);
        socket.emit("dm:message:replied", replyData);
      } catch (err) {
        console.error("[dm:message:reply] Error:", err);
      }
    });

    // Message Reply - Group
    socket.on("group:message:reply", async ({ parentMessageId, text, groupId } = {}) => {
      if (!parentMessageId || !text || !groupId) return;
      ...
      try {
        ...
        if (error) {
          console.error("[group:message:reply] Error saving reply:", error);
          return;
        }
        ...
        // Broadcast to group
        io.to(`group:${groupId}`).emit("group:message:replied", replyData);
      } catch (err) {
        console.error("[group:message:reply] Error:", err);
      }
    });
